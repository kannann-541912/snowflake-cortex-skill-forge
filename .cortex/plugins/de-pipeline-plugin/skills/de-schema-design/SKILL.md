...
# Client Context
Read `../../references/client-context.md` at the start of every invocation. This file controls
the **modeling paradigm** — `canonical`, `star_schema`, `data_vault`, or `one_big_table` —
and all associated naming conventions, SCD defaults, audit columns, type mapping overrides,
and PII masking strategy. If the file is absent or a value is unset (`~`), use the
built-in defaults. Never fail if the file is missing.

Key overrides to apply from this file:
- `modeling_paradigm` → switch between canonical/star schema vs Data Vault table structures
- `canonical.*` / `data_vault.*` → apply naming prefixes (STG_, HUB_, LNK_, SAT_, etc.)
- `audit_columns` → append client-standard audit columns to every table
- `pii_masking.default_strategy` → sha2 | null | partial_mask | tokenize
...
# Domain Context
You are a Snowflake schema architect specializing in translating raw source data profiles into
production-grade relational models. You know the TPCH schema thoroughly, understand SCD patterns,
and always produce schemas with governance-by-default: audit columns, masking policy references,
and lineage tags included from the start.
...
# When NOT to Use
- `profile_report.md` doesn't exist yet → run `de-profile` first
- User wants to generate DDL and deploy it → use `de-schema-setup` (this skill only designs, not deploys)
- User wants to add an existing Snowflake table to a project → use `snowflake-project-builder`

# Gotchas
- Always include audit columns (`_LOADED_AT`, `_SOURCE_SYSTEM`) in every generated table — never omit them.
- TPCH FLOAT prices must be cast to NUMBER(15,2) in the target — document this in the design.
- LINEITEM has a composite PK — generate a compound primary key constraint, not a single-column one.
- Do NOT use CREATE OR REPLACE in schema_design.md — it's a design document, not SQL.
- The `schema_design.md` must be committed to Git before invoking `de-schema-setup`.

# Standalone Quality Gate
```bash
test -f schema_design.md && echo "Schema design exists ✓" || echo "MISSING — run de-schema-design first"
```
...
- `modeling_paradigm` → determines the entire table structure (canonical vs data vault, etc.)
- `canonical.staging_prefix` / `canonical.mart_prefix` → override STG_, MART_ defaults
- `data_vault.*` → apply HUB_/LNK_/SAT_ naming and standard DV columns if paradigm = data_vault
- `canonical.default_scd_strategy` → override SCD type recommendation
- `audit_columns` → replace built-in `_LOADED_AT` / `_SOURCE_SYSTEM` with client standards
- `type_mapping_overrides` → apply any client-specific source→target type rules
- `pii_masking.*` → apply client masking strategy and visible roles
