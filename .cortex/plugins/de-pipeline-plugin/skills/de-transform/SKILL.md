...
defines as overrides: preferred transform engine (Dynamic Table vs Stream+Task vs dbt),
Dynamic Table lag and refresh mode, Task schedule, quality assertion thresholds, and
self-healing preferences. If the file is absent or a value is unset (`~`), use the
...
# Domain Context
You are a Snowflake transform engineer specializing in self-healing, incremental pipelines.
You prefer Dynamic Tables for declarative transforms and Streams+Tasks for stateful/side-effect
workflows. You always add post-transform quality assertions and enable automatic resume-on-failure.
...
# When NOT to Use
- Data hasn't been loaded yet → run `de-load-validate` first
- User only wants to design the transform SQL → use `de-transform-setup`
- User only wants dbt tests → use `dbt-expectations-generator`

# Gotchas
- Streams on `SNOWFLAKE_SAMPLE_DATA.TPCH_SF10` are useless — the dataset is static and emits no change events. Use Dynamic Tables for all TPCH mart transforms.
- Dynamic Tables are SUSPENDED by default — always `ALTER DYNAMIC TABLE ... RESUME` after creation.
- Tasks are SUSPENDED by default — always `ALTER TASK ... RESUME` after creation.
- Post-transform null rate > 5% on a NOT NULL mart column is a STOP — the transform SQL has a bug.
- `transform_mappings.yml` must exist before running this skill.

# Standalone Quality Gate
```sql
-- Verify mart table is populated
SELECT COUNT(*) AS mart_rows, MAX(_LOADED_AT) AS last_transform
FROM SANDBOX.TPCH.MART_{table_name}_ENRICHED;

-- Check for failed tasks
SELECT name, state, last_failed_run
...
- `preferred_engine` → override built-in Dynamic Table preference
- `dynamic_table.default_target_lag` → lag override (e.g. '15 minutes' for strict SLA)
- `stream_task.task_schedule` / `stream_task.stream_mode` → task and stream configuration
- `quality_assertions.*` → row delta warn %, freshness thresholds, null fail flag
- `self_healing.lookback_hours` → how far back to check for failed tasks
