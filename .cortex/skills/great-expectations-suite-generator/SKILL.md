...
defines as overrides: GX version (1.x vs legacy 0.x), project directory layout, expectation
thresholds (row count tolerance, numeric range buffer, mostly value), alerting (Slack webhook
env var), and which expectation types to generate. If the file is absent or a value is unset
(`~`), use the built-in defaults. Never fail if the file is missing.

# Great Expectations Suite Generator

## Domain Context
You are a Great Expectations v1.x specialist who generates complete, runnable GX suites for
Snowflake tables. You derive all thresholds from live data profiling and produce a suite that
can be run immediately with `checkpoint.run()`.

## When to Use
- User wants a Python-based Great Expectations test suite for a Snowflake table
- "generate GX suite", "create great expectations for this table", "add GX checks"
- User uses GX in their data stack and wants it integrated with Snowflake

## When NOT to Use
- User uses dbt → use `dbt-expectations-generator` instead (much simpler integration)
- User wants Snowflake-native DMF monitoring → use `dmf-generator`
- User only wants a one-time profile → use `de-profile`

## Gotchas
- Always target GX v1.x API — the v0.x and v1.x APIs are incompatible. Never use `context.add_expectation_suite()` (v0.x pattern).
- The Snowflake datasource connection string requires the `snowflake-sqlalchemy` package.
- Never hardcode credentials — always use environment variables or a Secrets Manager reference.
- Value set expectations (`expect_column_values_to_be_in_set`) break when the set is derived from a changing column — only use for truly stable categorical columns.
- The generated suite JSON must be committed to the GX `expectations/` directory, not the project root.
...
- `gx.version` → 1.x vs 0.x API (critical — incompatible APIs)
- `gx.expectations_dir` / `gx.datasources_dir` → output file paths
- `thresholds.*` → row count tolerance, numeric range buffer, mostly value, freshness
- `alerting.slack.*` → Slack webhook env var and notify_on setting
- `expectation_types.*` → enable/disable specific expectation categories
