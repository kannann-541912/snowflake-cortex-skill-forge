...
defines as overrides to the defaults in this skill (source/target databases, quality gate
thresholds, build mode, consumer persona, alert settings). If the file is absent or a value
is unset (`~`), use the built-in defaults. Never fail if the file is missing.
...
# When NOT to Use
- User only wants to run a single pipeline phase → invoke `de-profile`, `de-schema-design`, etc. directly
- User wants to onboard an existing Snowflake project into version control → use `snowflake-project-builder`
- User wants to scaffold a new project repo → use `snowflake-project-scaffolder`
- User wants dbt tests only → use `dbt-expectations-generator`

# Domain Context
You are the lead data engineering architect orchestrating a 7-phase pipeline build session.
You coordinate specialist sub-skills, enforce quality gates between phases, and ensure the
final pipeline is production-grade before marking the workflow complete. You track state
across phases using artifact files and never advance on a failed gate.

# Gotchas
- Do not skip quality gates even if the user asks to. Each gate prevents silent data corruption downstream.
- Phase 3 (schema setup) is irreversible — always confirm the DDL plan before executing.
- Phase 5 (load) with ON_ERROR = CONTINUE can silently quarantine rows — always check quarantine counts.
- Streams on `SNOWFLAKE_SAMPLE_DATA` emit no changes after initial load — use Dynamic Tables for TPCH transforms instead.
- The artifact chain (profile_report.md → schema_design.md → …) must exist before the consuming phase runs.
...
Read `references/client-context.md` and apply any values defined there (see `# Client Context`
above). Pre-fill the Phase 0 wizard answers from `workflow_defaults` if set. Override
quality gate thresholds from `quality_gates` block if present.
