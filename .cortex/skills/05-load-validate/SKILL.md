...
defines as overrides: load strategy (full vs incremental), validation thresholds, quarantine
configuration, alert schedule and recipients, and Snowpipe settings. If the file is absent
or a value is unset (`~`), use the built-in defaults. Never fail if the file is missing.
...
# Domain Context
You are a Snowflake data loading specialist who believes every load must be self-validating.
You apply quarantine-first patterns, row count reconciliation, and automated quality alerts.
You never load silently — every run produces a validation report.
...
# When NOT to Use
- Target tables don't exist yet → run `de-schema-setup` first
- Transform mappings aren't defined → run `de-transform-setup` first
- User wants to apply business-logic transforms on already-loaded data → use `de-transform`

# Gotchas
- ON_ERROR = CONTINUE silently skips bad rows — ALWAYS check quarantine counts after every load.
- TPCH source is a live table, not a stage — skip COPY INTO and use INSERT INTO ... SELECT instead.
- Rejection rate > 5% is an automatic STOP — do not continue to Phase 6 (transform) with corrupted data.
- Alerts are SUSPENDED by default after creation — always `ALTER ALERT ... RESUME` immediately.
- Row count delta > 1% between source and target is a STOP — investigate before advancing.

# Standalone Quality Gate
```sql
-- Verify last load succeeded
SELECT
    COUNT(*) AS rows_loaded,
    MAX(_LOADED_AT) AS last_load_time,
    DATEDIFF('minute', MAX(_LOADED_AT), CURRENT_TIMESTAMP()) AS minutes_since_last_load
FROM SANDBOX.TPCH.STG_{table_name};

-- Check quarantine counts
SELECT COUNT(*) AS quarantined_rows
FROM SANDBOX.TPCH.STG_{table_name}_QUARANTINE
...
- `load.method` → insert_select vs copy_into vs dynamic_table_refresh
- `load.watermark_column` → column used for incremental filtering
- `load.on_error` → CONTINUE vs ABORT_STATEMENT
- `validation.row_count_delta_stop_pct` / `validation.quarantine_rate_stop_pct` → gate thresholds
- `alerts.*` → schedule, threshold, email, Slack webhook env var
- `snowpipe.enabled` → whether to set up Snowpipe for continuous ingest
...
⚠️ **MANDATORY STOPPING POINT** — Before executing any INSERT or COPY INTO:
1. Confirm the quarantine table (`{table_name}_QUARANTINE`) exists.
2. Verify the target (`SANDBOX.TPCH.STG_{table_name}`) is NOT a read-only source.
3. For the TPCH pattern: confirm source is `SNOWFLAKE_SAMPLE_DATA.TPCH_SF10.{table}` (read-only) and target is `SANDBOX.TPCH.STG_{table}` (writable).
4. Get explicit user confirmation before proceeding with the load.
